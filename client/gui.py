...
#####################################################
	def show (self, wm):
		...
		if wm.mytank.tanktype == basictypes.TankTypes.attacker and not mytank_is_alive:
			respawn_time = int (wm.mytank.respawn_time)
			self.screen.blit (pygame.transform.scale (self.digits [respawn_time] , ( (self.monitor_height - 2*self.offset_height)//2, (self.monitor_height - 2*self.offset_height))), (self.monitor_width//2 - ((self.monitor_height - 2*self.offset_height) // 4), self.offset_height))
		#updating!
