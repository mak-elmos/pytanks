...
class QLearningAI:
	...
	"""def calculateFitness(self, state):
		...
		h = state.bombPos.y - (state.myPos.y + self.map_attacker_height / 2.0)
		if h < 0.0:
			endX = state.bombPos.x
		else:
		...
		if abs(state.myPos.x - endX) < self.map_attacker_width / 2.0 + self.map_bomb_radius:
			print "------------------------------------------------------------------------------------"
			fitness -= 1000
	...
	def calculateFitness(self, state):
		...
		if abs(state.myPos.x - state.bombPos.x) < self.map_attacker_width / 2.0 + self.map_bomb_radius + 0.1 and abs(state.myPos.y - state.bombPos.y) < self.map_attacker_height / 2.0 + self.map_bomb_radius + 0.25:
			print "------------------------------------------------------------------------------------"
			fitness -= 1000
	...
	def pre_process(self, wm):
		if self.currentState == None:
			self.counter = -1
			self.currentState = State()
			self.currentState.position = Vector(100, 100)
			self.currentAction = basictypes.Actions.none

		wms = self.getWorldModelState(wm)
		if wms.toStr() != self.currentState.toStr():
			self.currentState  = wms
			if not self.currentState.toStr() in self.q_data:
				self.addState(self.currentState)

			## entire random policy
			"""self.counter += 1
			if self.counter >= 15:
				self.counter = 0

			if self.counter == 0:
				self.currentAction = self.getRandomAction()
				while not self.currentAction.move in self.q_data[self.currentState.toStr()]:
					 self.currentAction = self.getRandomAction()"""
			...
			if self.counter == 0:
				...
				print self.q_data[self.currentState.toStr()]

			## best move policy
			"""self.currentAction = Action()
			self.currentAction.move = self.getBestMove(wm)"""
	...
	def process(self, wm):
		...
		self.q_data[currentState.toStr()][currentAction.move] += self.alpha * (self.calculateFitness(currentState) + self.gamma * self.getMaxRewardOfState(nextState) - self.q_data[currentState.toStr()][currentAction.move])
